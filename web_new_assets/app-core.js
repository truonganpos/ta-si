...
// Đổi 1 lần tại đây để cập nhật số liên hệ và Zalo toàn web.
    ...
    const syncCatalogFromPosCache = (options = {}) => {
        try {
            ...
            return window.retailFirebase.loadCatalogPage({
                ...
                shopProducts = [];
                productFeedState.error = 'Đang tải danh sách sản phẩm';
                if(options.refreshViews !== false) refreshCatalogViews();
                ...
                productFeedState.error = message.includes('permission_denied')
                    ? 'Không đọc được danh sách sản phẩm từ database. Hãy kiểm tra danh sách sản phẩm.'
                    : 'Lỗi tải sản phẩm từ Database.';
                refreshCatalogViews();
                return false;
            }).finally(() => {
                productFeedState.loading = false;
            ...
            console.warn('Không đồng bộ được catalog từ database:', error);
            shopProducts = [];
            productFeedState.error = 'Lỗi tải sản phẩm từ database.';
            refreshCatalogViews();
            return false;
        }
    };
    const syncCatalogDeltaFromCloud = async () => {
        ...
        try {
            ...
            console.warn('Khong tai duoc delta catalog database:', error);
            if(!shopProducts.length) {
                const message = String((error && (error.code || error.message)) || '').toLowerCase();
                productFeedState.error = message.includes('permission_denied')
                    ? 'Không đọc được danh sách sản phẩm từ database. Hãy kiểm tra dữ liệu sản phẩm.'
                    : 'Lỗi tải sản phẩm từ database.';
                refreshCatalogViews();
            }
        } finally {
    ...
    function renderProductsList(arr, containerId = 'product-container', sourceArray = shopProducts) {
        ...
        const emptyMessage = productFeedState.error
            ? `<div class='${emptyClass} text-center py-10 text-red-400'><i class="fa-solid fa-circle-exclamation text-3xl mb-3 block opacity-80"></i><p>${productFeedState.error}</p><p class='text-xs text-gray-400 mt-2'>Sản phẩm đang được nạp mới nhất.</p></div>`
